...
// List codec without specialization. The upstream crate used
// `#![feature(specialization)]` to overlay `Vec<i8>`/`Vec<bool>` raw-bytes
// fast paths on top of this generic impl. Those fast paths are unused by DTV
// (Huya only ever encodes `Vec<String>` lists, via this generic path), so the
// concrete impls were dropped and `default` removed to compile on stable.
