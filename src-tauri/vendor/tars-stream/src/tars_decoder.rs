...
// List codec without specialization (mirrors tars_encoder.rs). The upstream
// `default fn` + i8/bool fast-path overrides required `#![feature(specialization)]`;
// DTV never decodes a `Vec<i8>`/`Vec<bool>` Tars list, so the concrete impls were
// dropped and `default` removed to compile on stable.
...
mod tests {
    ...
    fn test_decode_list() {
        ...
        let b: [u8; 28] = [
            0xa9, 0x02, size[0], size[1], size[2], size[3], // {tag: 0, type: 6}
            0x06, 7, b'f', b'o', b'o', b' ', b'b', b'a', b'r', // {tag: 0, type: 6}
            0x06, 11, b'h', b'e', b'l', b'l', b'o', b' ', b'w', b'o', b'r', b'l', b'd',
        ];
        let mut de = TarsDecoder::from(&b[..]);
