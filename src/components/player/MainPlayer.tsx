...
function resolveStoredQuality(platform: Platform) {
  try {
    ...
    if (saved && (qualityOptions as readonly string[]).includes(saved))
      return saved;
  } catch {
    // ignore
  }
...
function isOfflineMessage(msg: string) {
  ...
  return (
    s.includes('未开播') || s.includes('主播未开播') || s.includes('房间不存在')
  );
}
...
function resolveCurrentLineFor(
  options: LineOption[],
  currentLine: string | null,
) {
  if (!options.length) return null;
  if (currentLine && options.some((o) => o.key === currentLine))
    return currentLine;
  return options[0]?.key ?? null;
}
...
export function MainPlayer({
  ...
  const [currentLine, setCurrentLine] = useState<string | null>(() =>
    typeof window === 'undefined'
      ? null
      : resolveStoredLine(platform, lineOptionsByPlatform[platform] ?? []),
  );
  ...
  const [danmuKeywordBlock, setDanmuKeywordBlock] =
    useState<DanmuKeywordBlockPreferences>(() => {
      ...
      return loaded
        ? { enabled: true, keywords: loaded.keywords }
    ...
    async (
      ...
      if (isHlsPlayback) {
        ...
        if (
          ...
          !supportsMseType(hev1) &&
          supportsMseType(hvc1)
        ) {
          try {
            const mod: any =
      ...
      linePluginRef.current = player.registerPlugin?.(LineControl, {
        ...
        getCurrentLabel: () => {
          const key = resolveCurrentLineFor(
            lineOptionsRef.current,
            currentLineRef.current,
          );
          return (
            lineOptionsRef.current.find((o) => o.key === key)?.label ?? '线路'
          );
        },
    ...
    async (
      ...
      try {
        if (platform === Platform.DOUYU) {
          const resolvedLine = resolveCurrentLineFor(
            lineOptions,
            effectiveLine,
          );
          try {
  ...
  useEffect(() => {
    try {
      ...
      linePluginRef.current?.updateLabel?.(
        lineOptions.find(
          (o) => o.key === resolveCurrentLineFor(lineOptions, currentLine),
        )?.label ?? '线路',
      );
    } catch {
      // ignore
            ...
            <div className="video-container">
              <div className="player-topbar">
                <div className="player-topbar-left">
                  ...
                  <div
                    className="player-topbar-streamer"
                    title={playerTitle || roomId}
                  >
                    <div className="player-topbar-avatar">
                      {playerAvatar ? (
                        // eslint-disable-next-line @next/next/no-img-element
                        <img
                          src={getAvatarSrc(platform, playerAvatar)}
                          alt={playerAnchorName ?? roomId}
                        />
                      ) : (
                        <div className="player-topbar-avatarFallback">
                          {(playerAnchorName || roomId || 'D')
                            .charAt(0)
                            .toUpperCase()}
                        </div>
                      )}
