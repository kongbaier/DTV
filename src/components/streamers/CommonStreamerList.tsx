...
export function CommonStreamerList({
  ...
  const loadMoreGateRef = useRef<{
    key: string;
    lastLen: number;
    lastAt: number;
  }>({ key: '', lastLen: -1, lastAt: 0 });
  ...
  const huya = useHuyaLiveRooms(
    platform === 'huya' ? resolvedSubcategoryId : null,
    { defaultPageSize: defaultPageSize ?? 120 },
  );
  ...
  const douyu = useDouyuLiveRooms(
    platform === 'douyu' ? douyuCategoryType : null,
    platform === 'douyu' ? douyuCategoryId : null,
  );
  ...
  const listKey = useMemo(() => {
    if (platform === 'douyu')
      return `douyu:${douyuCategoryType ?? 'none'}:${douyuCategoryId ?? 'none'}`;
  ...
  useEffect(() => {
    ...
    if (rootEl.clientHeight <= 0 || rootEl.getClientRects().length === 0)
      return;
        ...
        <div className={styles.noStreamers} style={{ maxWidth: 720 }}>
          ...
          <div style={{ marginTop: 14 }}>
            <button
              type="button"
              className="retry-btn"
              onClick={() => void (selected as any).loadInitialRooms?.()}
            >
              重试
            </button>
          </div>
  ...
  return (
    <div className={styles.container}>
      <div ref={scrollRef} className={styles.scrollArea} onScroll={onScroll}>
                  ...
                  <div className={styles.preview}>
                    <div className={styles.imageWrapper}>
                      ...
                      <div
                        ...
                        aria-label={`观看人数 ${room.viewer_count_str || '0'}`}
                      >
