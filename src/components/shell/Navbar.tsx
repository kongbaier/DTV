  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (cancelled) return;
        const local =
          typeof res?.local_version === 'string' ? res.local_version : '';
        ...
        if (
          ...
          const info: VersionInfo = {
            ...
            notes: Array.isArray(remote.notes)
              ? remote.notes.filter((x: any) => typeof x === 'string')
              : undefined,
          ...
          >
            <div className={styles.playerIslandLeft}>
              {island.avatarUrl ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img
                  className={styles.playerIslandAvatar}
                  src={island.avatarUrl}
                  alt={islandDisplayName}
                />
              ) : (
                <div
                  className={`${styles.playerIslandAvatar} ${styles.playerIslandAvatarFallback}`}
                >
              ...
              <div className={styles.playerIslandMeta}>
                <div
                  className={styles.playerIslandName}
                  title={islandDisplayName}
                >
                  {islandDisplayName}
                </div>
                <div
                  className={styles.playerIslandTitle}
                  title={islandDisplayTitle}
                >
                  {islandDisplayTitle}
                </div>
              </div>
                  ...
                  island.platform === Platform.DOUYU
                    ...
                    : island.platform === Platform.DOUYIN
                      ...
                      : island.platform === Platform.HUYA
                        ? 'HUYA'
                        : 'BILIBILI';
                if (follow.isFollowed(fp, island.roomId))
                  follow.unfollowStreamer(fp, island.roomId);
                else {
      ...
      <div className={styles.actions} data-drag-region>
        ...
        {isWindows ? (
          ...
          >
            ...
            >
              {isMaximized ? (
                <WinCaptionRestoreIcon />
              ) : (
                <WinCaptionMaximizeIcon />
              )}
            </button>
            <button
              type="button"
