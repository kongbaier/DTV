...
export function LanSyncModal({
  ...
  const startShare = useCallback(async () => {
    ...
    try {
      const payload = createLanSyncPayload(window.localStorage, {
        client: 'desktop',
        appVersion: appVersion || null,
      });
      const res = (await invoke<any>('lan_sync_start_server', {
        payload,
      })) as ServerInfo;
      setServerInfo(res);
    } catch (e) {
  ...
  const exportToDesktopJson = useCallback(async () => {
    ...
    try {
      ...
      const savedPath = (await invoke<string>(
        'export_lan_sync_json_to_desktop',
        {
          contents,
  ...
  const importFromJsonFile = useCallback(async () => {
    ...
    try {
      const picked = (await invoke<any>('pick_lan_sync_json_import')) as {
        path: string;
        content: string;
      } | null;
      ...
      const result = applyIncrementalLanSyncImport(
        window.localStorage,
        payload.entries,
      );
      const extraParts: string[] = [];
      if (typeof result.addedCustomCategories === 'number')
        extraParts.push(`自定义分区新增 ${result.addedCustomCategories}`);
      if (typeof result.addedDanmuBlockKeywords === 'number')
  ...
  const importFromDiscoveredPeers = useCallback(async () => {
    ...
    try {
      const peers = (await invoke<any>('lan_sync_discover', {
        timeoutMs: 1600,
      })) as DiscoveredPeer[];
      ...
      const settled = await Promise.allSettled(
        peers.map(async (peer) => {
          ...
          const manifest = (await invoke<any>('lan_sync_fetch_manifest', {
      ...
      settled.forEach((item, idx) => {
        ...
        const reasonMsg =
          item.reason instanceof Error
            ? item.reason.message
            : String(item.reason || 'Failed to fetch');
      ...
      if (!candidates.length) {
        ...
        throw new Error(
          `未找到可导入的数据：共享端可能已关闭、token 不匹配，或 mDNS 解析到了不可达 IP（如 169.254.*）。${suffix}`,
        );
      }
      candidates.sort((a, b) => b.exportedAt - a.exportedAt);
      const best = candidates[0];

      const payloadRaw = (await invoke<any>('lan_sync_fetch_payload', {
        baseUrl: best.peer.baseUrl,
        token: best.token,
      })) as any;
  ...
  const confirmImport = useCallback(async () => {
    ...
    setImportResult(null);
    try {
      const { baseUrl, token } = normalizeImportTarget(
        manualTarget,
        DEFAULT_TOKEN,
      );
      const payloadRaw = (await invoke<any>('lan_sync_fetch_payload', {
      ...
      >
        ...
        >
          <div className={styles.overlayHeader}>
            ...
            <button
              type="button"
              className={`${styles.overlayClose} ${styles.syncBtnGlass}`}
              onClick={() => close()}
              aria-label="关闭"
            >
              <X size={16} />
          ...
          <div className={styles.overlayBody}>
            ...
            {tab === 'export' ? (
                ...
                {serverInfo?.hosts?.length && bestHost ? (
                  <div className={styles.syncMetaGrid} aria-label="share-meta">
                    ...
                    <div className={styles.syncMetaItem}>
                      ...
                      <div className={styles.syncMetaVal}>
                        {serverInfo?.port || FIXED_PORT}
                      </div>
                    </div>
                  </div>
                ) : null}
              ...
              <>
                <div
                  className={styles.syncActionsRow}
                  aria-label="json-import-actions"
                >
                  <button
                    type="button"
                ...
                {importResult ? (
                  <p className={styles.syncHint}>{importResult}</p>
                ) : null}
                ...
                <div className={styles.syncActionsRow}>
                  <button
                    ...
                    disabled={
                      importBusy || !manualTarget.trim() || !canUseStorage
                    }
                  >
                    导入
                  </button>
