...
type FollowContextValue = {
  ...
  removeStreamerFromFolder: (
    folderId: string,
    platform: Platform,
    id: string,
  ) => void;
  removeStreamerFromFolderByKey: (
    streamerKey: string,
    folderId: string,
  ) => void;
  updateStreamer: (
    platform: Platform,
    id: string,
    patch: Partial<FollowedStreamer>,
  ) => void;
};
...
function normalizeListOrder(order: FollowListItem[]): FollowListItem[] {
  ...
  for (const item of order) {
    ...
    const key = normalizeStreamerKey(
...
export function FollowProvider({ children }: { children: React.ReactNode }) {
  ...
  const folders = useMemo(
    () =>
      listOrder
        .filter(
          (x): x is { type: 'folder'; data: FollowFolder } =>
            x.type === 'folder',
        )
        .map((x) => x.data),
    [listOrder],
  ...
  const updateStreamer = useCallback(
    (platform: Platform, id: string, patch: Partial<FollowedStreamer>) => {
      const key = `${platform}:${id}`;
      setFollowedStreamers((prev) =>
        prev.map((s) =>
          `${s.platform}:${s.id}` === key ? { ...s, ...patch } : s,
        ),
      );
      setListOrder((prev) =>
        prev.map((item) => {
          ...
          return {
            ...item,
            data: { ...item.data, streamerIds: nextStreamerIds },
          };
        }),
      );
    },
  ...
  const value = useMemo<FollowContextValue>(() => {
    return {
      ...
      isFollowed: (platform, id) =>
        followedStreamers.some((s) => s.platform === platform && s.id === id),
      followStreamer: (streamer) => {
        setFollowedStreamers((prev) => {
          if (
            prev.some(
              (s) => s.platform === streamer.platform && s.id === streamer.id,
            )
          )
            return prev;
        ...
        setListOrder((prev) => {
          ...
          if (
            prev.some(
              (item) =>
                item.type === 'streamer' &&
                `${item.data.platform}:${item.data.id}` === key,
            )
          )
            return prev;
          return normalizeListOrder([
            ...prev,
            { type: 'streamer', data: streamer },
          ]);
        });
      },
      unfollowStreamer: (platform, id) => {
        const key = `${platform}:${id}`;
        setFollowedStreamers((prev) =>
          prev.filter((s) => !(s.platform === platform && s.id === id)),
        );
        setListOrder((prev) =>
          normalizeListOrder(
            prev
              .map((item) => {
                if (item.type === 'folder') {
                  return {
                    ...
                    data: {
                      ...item.data,
                      streamerIds: item.data.streamerIds.filter(
                        (x) => x !== key,
                      ),
                    },
                  };
          ...
          normalizeListOrder(
            prev.map((item) =>
              item.type === 'folder' && item.data.id === folderId
                ? { ...item, data: { ...item.data, name: trimmed } }
                : item,
            ),
          ),
        );
      ...
      deleteFolder: (folderId) => {
        setListOrder((prev) => {
          const idx = prev.findIndex(
            (x) => x.type === 'folder' && x.data.id === folderId,
          );
          if (idx < 0) return prev;
          const folder = prev[idx] as any as {
            type: 'folder';
            data: FollowFolder;
          };
          ...
          normalizeListOrder(
            prev.map((item) =>
              item.type === 'folder' && item.data.id === folderId
                ? {
                    ...item,
                    data: { ...item.data, expanded: !item.data.expanded },
                  }
                : item,
            ),
          ),
          ...
          const next = prev
            .filter(
              (item) =>
                !(
                  item.type === 'streamer' &&
                  normalizeStreamerKey(`${item.data.platform}:${item.data.id}`)
                    .key === norm.key
                ),
            )
            .map((item) => {
              ...
              const nextIds = item.data.streamerIds.filter(
                (id) => normalizeStreamerKey(id).key !== norm.key,
              );
              if (item.data.id !== folderId) {
                return nextIds.length === item.data.streamerIds.length
                  ? item
                  : { ...item, data: { ...item.data, streamerIds: nextIds } };
              }

              if (
                !nextIds.some((id) => normalizeStreamerKey(id).key === norm.key)
              )
                nextIds.push(norm.key);
              return {
                ...item,
                data: { ...item.data, streamerIds: nextIds, expanded: true },
              };
            });
            ...
            .map((item) => {
              if (item.type === 'folder' && item.data.id === folderId) {
                ...
                return {
                  ...
                  data: {
                    ...item.data,
                    streamerIds: [...item.data.streamerIds, key],
                    expanded: true,
                  },
                };
              }
      ...
      removeStreamerFromFolder: (folderId, platform, id) => {
        ...
        setListOrder((prev) => {
          const folderIndex = prev.findIndex(
            (x) => x.type === 'folder' && x.data.id === folderId,
          );
          if (folderIndex < 0) return prev;

          const streamer = followedStreamers.find(
            (s) =>
              normalizeStreamerKey(`${s.platform}:${s.id}`).key === norm.key,
          );
          const existsInTop = prev.some(
            (x) =>
              x.type === 'streamer' &&
              normalizeStreamerKey(`${x.data.platform}:${x.data.id}`).key ===
                norm.key,
          );
          ...
          const next = prev.map((item) => {
            if (item.type !== 'folder' || item.data.id !== folderId)
              return item;
            const nextIds = item.data.streamerIds.filter(
              (x) => normalizeStreamerKey(x).key !== norm.key,
            );
            return nextIds.length === item.data.streamerIds.length
              ? item
              : { ...item, data: { ...item.data, streamerIds: nextIds } };
