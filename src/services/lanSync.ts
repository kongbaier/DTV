...
export function createLanSyncPayload(
  storage: Storage,
...
export function parseLanSyncManifest(raw: unknown): LanSyncManifest {
  ...
  if (kind && kind !== LAN_SYNC_KIND)
    throw new Error('This is not a DTV LAN sync export.');
  if (version != null && version !== LAN_SYNC_VERSION)
    throw new Error(`Unsupported sync version: ${String(raw.version)}.`);

  if (
    typeof raw.exportedAt !== 'string' ||
    Number.isNaN(Date.parse(raw.exportedAt))
  )
    throw new Error('Manifest exportedAt is invalid.');
  if (
    !isRecord(raw.source) ||
    typeof raw.source.client !== 'string' ||
    !raw.source.client.trim()
  )
    throw new Error('Manifest source is invalid.');
  if (!isRecord(raw.summary)) throw new Error('Manifest summary is missing.');

  const summary = raw.summary;
  const num = (key: keyof LanSyncSummary) =>
  ...
  return {
    ...
    source: {
      ...
      appVersion:
        typeof raw.source.appVersion === 'string'
...
export function parseLanSyncPayload(raw: unknown): LanSyncPayload {
  ...
  if (
    ...
    throw new Error('Payload exportedAt is invalid.');
  if (
    !isRecord(raw.source) ||
    typeof raw.source.client !== 'string' ||
    !raw.source.client.trim()
  )
    throw new Error('Payload source is invalid.');
...
export function applyIncrementalLanSyncImport(
  storage: Storage,
  remoteEntries: PortableEntries,
): LanSyncImportResult {
  const remoteFoldersFromKey = safeParseJsonArray<FollowFolder>(
    remoteEntries['followFolders'],
  );
  const remoteStreamersFromKey = safeParseJsonArray<FollowedStreamer>(
    remoteEntries['followedStreamers'],
  );
  const remoteOrderFromKey = safeParseJsonArray<FollowListItem>(
    remoteEntries['followListOrder'],
  );
  const remoteCustomCategoriesFromKey = safeParseJsonArray<{ key: string }>(
    remoteEntries['dtv_custom_categories_v1'],
  );
  const remoteDanmuBlockKeywordsFromKey = safeParseJsonArray<string>(
    remoteEntries['danmu_block_keywords'],
  );
  const remoteDanmuPreferencesFromKey = safeParseJsonObject(
    remoteEntries['dtv_danmu_preferences_v1'],
  );

  const localFollowed = safeParseJsonArray<FollowedStreamer>(
    storage.getItem('followedStreamers'),
  );
  const localFolders = safeParseJsonArray<FollowFolder>(
    storage.getItem('followFolders'),
  );
  const localOrder = safeParseJsonArray<FollowListItem>(
    storage.getItem('followListOrder'),
  );
  const localCustomCategories = safeParseJsonArray<{ key: string }>(
    storage.getItem('dtv_custom_categories_v1'),
  );
  const localDanmuBlockKeywords = safeParseJsonArray<string>(
    storage.getItem('danmu_block_keywords'),
  );
  ...
  const addStreamerIfMissing = (
    platform: string,
    id: string,
    streamer?: Partial<FollowedStreamer>,
  ) => {
  ...
  const existingFolderIds = new Set(
    localFolders.map((f) => String(f?.id || '')).filter(Boolean),
  );
  ...
  const mergedFolders: FollowFolder[] = [
    ...localFolders.map((f) => ({
      ...f,
      streamerIds: Array.isArray(f.streamerIds) ? [...f.streamerIds] : [],
    })),
  ];
  ...
  for (const rf of remoteFolders) {
    ...
    if (match && match.id) {
      ...
      const existing = new Set(
        target.streamerIds.map((x) => normalizeStreamerKey(String(x)).key),
      );
  ...
  const localFolderOrder = (Array.isArray(localOrder) ? localOrder : []).filter(
    (x): x is Extract<FollowListItem, { type: 'folder' }> =>
      x?.type === 'folder',
  );
  const localStreamerOrder = (
    Array.isArray(localOrder) ? localOrder : []
  ).filter(
    (x): x is Extract<FollowListItem, { type: 'streamer' }> =>
      x?.type === 'streamer',
  );

  const folderIdsInOrder = new Set(
    localFolderOrder.map((x) => String(x?.data?.id || '')).filter(Boolean),
  );
  ...
  for (const item of localStreamerOrder) {
    ...
    const merged = streamerByKey.get(
      `${String(s.platform).toUpperCase()}:${String(s.id)}`,
    );
  ...
  const mergedOrder: FollowListItem[] = [
    ...updatedFolderItems,
    ...updatedStreamerItems,
  ];

  // Merge custom categories by key (incremental).
  const localCategories = Array.isArray(localCustomCategories)
    ? localCustomCategories
    : [];
  const remoteCategories = Array.isArray(remoteCustomCategoriesFromKey)
    ? remoteCustomCategoriesFromKey
    : [];
  ...
  const remoteKeywords = normalizeDanmuBlockKeywords(
    remoteDanmuBlockKeywordsFromKey,
  );
  const mergedKeywords = normalizeDanmuBlockKeywords([
    ...localKeywords,
    ...remoteKeywords,
  ]);
  addedDanmuBlockKeywords = Math.max(
    0,
    mergedKeywords.length - localKeywords.length,
  );

  // Apply danmu preferences only when local missing/invalid (incremental).
  const canApplyRemotePreferences =
    !localDanmuPreferences && !!remoteDanmuPreferencesFromKey;
  ...
  if (mergedKeywords.length)
    storage.setItem('danmu_block_keywords', JSON.stringify(mergedKeywords));
  if (canApplyRemotePreferences) {
    storage.setItem(
      'dtv_danmu_preferences_v1',
      JSON.stringify(remoteDanmuPreferencesFromKey),
    );
